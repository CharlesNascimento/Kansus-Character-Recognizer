...
public class DocumentScanner {
	...
	public DocumentScanner(ProgressListener progressListener) {
	 ...
	 *            image.
	 */
	public final void scan(PixelImage pixelImage, DocumentScannerListener listener, int blockX1, int blockY1, int blockX2,
			int blockY2) {
		...
		for (int i = 0; (i + 1) < al.size(); i += 2) {
			...
			this.progressListener.onProgressChanged("Extraindo caracteres...", progress);
	...
	private void processRow(PixelImage pixelImage, DocumentScannerListener listener, int[] pixels, int w, int h, int x1,
		...
		for (int x = x1 + 1; x < (x2 - 1); x++) {
			...
			if (isWhiteSpace) {
				numConsecutiveWhite++;
				// System.out.println(numConsecutiveWhite + ", " + minCharBreakWidth);
